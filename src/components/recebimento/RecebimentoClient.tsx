...
export function RecebimentoClient({ notasIniciais }: RecebimentoClientProps) {
  ...
  function exportar(formato: "xlsx" | "pdf") {
    ...
    if (formato === "xlsx") {
      const bytes = gerarRecebimentoXlsx(dados);
      baixarBlob(
    ...
    startTransition(async () => {
      ...
      baixarBlob(new Blob([bytes as BlobPart], { type: "application/pdf" }), "recebimento-nf.pdf");
    });
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-muted-foreground">{notas.length} nota(s) registrada(s).</p>
        <div className="flex gap-2">
          <button type="button" onClick={() => exportar("xlsx")} disabled={notas.length === 0} className={botaoSecundario}>
      ...
      {erro ? <p className="rounded-lg bg-danger-dim px-3 py-2 text-sm text-danger">{erro}</p> : null}

      <div className="rounded-xl border border-border bg-card p-4">
        <h3 className="mb-3 text-sm font-semibold text-card-foreground">Nova NF</h3>
      ...
      {notas.length === 0 ? (
        ...
        grupos.map((grupo) => (
          <section key={grupo.chave} className="flex flex-col gap-4">
            <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">{grupo.rotulo}</h2>
            {grupo.itens.map((nota) => (
              <div key={nota.id} className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <p className="text-sm font-semibold text-card-foreground">
                        NF {nota.numero} · {nota.fornecedor}
                      </p>
                      <p className="text-xs text-muted-foreground">Emitida em {dataBr(nota.dataEmissao)}</p>
                    </div>
                    <div className="flex gap-2">
                ...
                <div className="mt-3 overflow-x-auto">
                    ...
                    <thead>
                      <tr className="border-b border-border text-left text-xs text-muted-foreground">
                        ...
                        {RECEBIMENTO_EVENTOS.map((evento) => {
                          ...
                          return (
                            <th key={evento} className="w-20 py-1.5 text-center font-medium">
                    ...
                    <tbody>
                      {nota.itens.map((item) => (
                        <tr key={item.id} className="border-b border-border/60">
                          <td className="py-1.5 pr-2 text-card-foreground">{item.produto}</td>
                          {RECEBIMENTO_EVENTOS.map((evento) => (
                            <td key={evento} className="text-center">
                              <input
                                type="checkbox"
                                checked={item[evento]}
                  ...
                  {nota.itens.length === 0 ? <p className="py-2 text-xs text-muted-foreground">Nenhum produto ainda.</p> : null}
                </div>
