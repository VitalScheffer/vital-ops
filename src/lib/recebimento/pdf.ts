...
export async function gerarRecebimentoPdf(notas: readonly NotaRecebimentoExport[], geradoEm: string): Promise<Uint8Array> {
  ...
  const escrever = (
    ...
    x: number,
    baseline: number,
    o: { size?: number; bold?: boolean; cor?: ReturnType<typeof rgb>; alinhar?: "dir" | "centro"; larguraMax?: number } = {},
  ) => {
    const size = o.size ?? 9;
  ...
  const desenharLogo = (x: number, yTopo: number, altura: number) => {
    try {
      ...
      const opt = { x, y: yTopo, scale: escala, borderColor: COR.turquesa, borderWidth: 3.2 * escala, borderLineCap: LineCapStyle.Round };
  ...
  const desenharCabecalho = () => {
    ...
    escrever("Recebimento de NF", DIR, A4.altura - 32, { size: 13, bold: true, cor: COR.branco, alinhar: "dir" });
    escrever(`Gerado em ${geradoEm}`, DIR, A4.altura - 47, { size: 8.5, cor: COR.aguaClara, alinhar: "dir" });
  ...
  const novaPagina = () => {
    ...
    escrever("Vital Scheffer · Vital Ops · Recebimento de NF", MARGEM + 30, A4.altura - 24, { size: 9, bold: true, cor: COR.branco });
    y = A4.altura - 40 - 16;
  };
  ...
  const quadradinho = (x: number, baseline: number, marcado: boolean) => {
    ...
    pagina.drawRectangle({
      ...
      width: lado,
      height: lado,
      color: marcado ? COR.turquesa : COR.branco,
